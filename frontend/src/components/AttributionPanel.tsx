...
export function AttributionPanel({ claimed, report }: Props) {
  return (
    <div className="panel p-4">
      <h2 className="text-sm font-semibold uppercase tracking-wide text-slate-400">Claimed Provenance</h2>
      <div className="mt-3 space-y-3">
        {claimed.map((claim) => (
          <div key={claim.memory_id} className="space-y-1">
            <div className="flex items-center justify-between text-sm">
              <span className="font-semibold text-slate-100">{claim.display_id}</span>
            ...
            <p className="text-xs leading-4 text-slate-500">{claim.explanation}</p>
          </div>
        ))}
      </div>
      {report ? (
